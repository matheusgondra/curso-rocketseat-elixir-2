defmodule Exlivery.Orders.CreateOrUpdateTest do
  ...
  describe "call/1" do
    ...
    test "when there is no user with given cpf, returns an error", %{item1: item1, item2: item2} do
      params = %{user_cpf: "9999999999", items: [item1, item2]}
    ...
    test "when there are invalid items, returns an errorr", %{
      user_cpf: cpf,
      item1: item1,
      item2: item2
    } do
      ...
      expected_response = {:error, "Invalid item"}

      assert response == expected_response
    end

    test "when there are no items, returns an errorr", %{user_cpf: cpf} do
